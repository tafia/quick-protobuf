...
use crate::types::{
    Enumerator, Field, FieldType, FileDescriptor, Frequency, Message, OneOf, Proto2Frequency,
    Proto3Frequency, RpcFunctionDeclaration, RpcService, Syntax,
};
    ...
    for &(k, v) in key_vals.iter() {
        if k == "default" {
            return match (syntax, typ) {
                ...
                (Syntax::Proto3, _) => Err("Custom default values not allowed in Proto3!"),
            };
        }
    }
    Ok(None)
...
impl TryFrom<(Syntax, FieldType, Option<ParsingStageFrequencyToken>)> for Frequency {
    ...
    fn try_from(value: (Syntax, FieldType, Option<ParsingStageFrequencyToken>)) -> Result<Self, Self::Error> {
        match value {
            ...
            (Syntax::Proto3, _, Some(ParsingStageFrequencyToken::Required)) => Err(r#"Proto3 does not support "required" fields!"#),
...
fn message_event(syntax: Syntax) -> impl FnMut(&str) -> IResult<&str, MessageEvent> {
    move |input | {
        alt((
            ...
            map(enumerator(syntax), MessageEvent::Enumerator),
...
fn enumerator(syntax: Syntax) -> impl FnMut(&str) -> IResult<&str, Enumerator> {
    move |input| {
        map_res(
            terminated(
                pair(
            ...
            |(name, fields)|{
                if syntax == Syntax::Proto3 && !fields.is_empty() && fields[0].1 != 0 {
                    return Err("In Proto3, the first defined field must have key 0!")
                }
...
fn scan_syntax(input: &str) -> IResult<&str, Syntax> {
    map_res(separated_list0(many0(anychar), syntax), |v| {
        if v.len() > 1 {
            return Err("More than 1 syntax declaration detected!");
        }
    ...
    let parser = move |input: &'a str| -> IResult<&'a str, FileDescriptor, nom::error::Error<&str>> {
        map(
            many0(alt((
                ...
                map(enumerator(got_syntax), Event::Enum),
...
mod test {
    ...
    fn test_enum() {
        test_syntaxes(move |syntax: Syntax| -> Result<(), &str> {
            ...
            let mess = enumerator(syntax)(msg);
            if let ::nom::IResult::Ok((_, mess)) = mess {
    ...
    fn enum_comments() {
        test_syntaxes(move |syntax: Syntax| -> Result<(), &str> {
            ...
            let en = assert_complete(enumerator(syntax)(msg))?;
    ...
    fn test_missing_tokens() {
        ...
        fn test_missing_tokens_enum(syntax: Syntax) -> Result<(), &'static str> {
            /* Check that base case is actually correct before we start deleting tokens */
            result_assert(enumerator(syntax)(
                r#"enum A {
                    FIELD_A = 0;
                }"#
            )
        ...
        test_syntaxes(move |syntax: Syntax| -> Result<(), &str> {
            ...
            test_missing_tokens_enum(syntax)?;
    ...
    fn test_which_names_can_be_qualified() {
        ...
        fn test_which_names_can_be_qualified_enum(syntax: Syntax) -> Result<(), &'static str> {
            /* Check that base case is actually correct before we start qualifying names */
            result_assert(enumerator(syntax)(
                r#"enum A {
                    enum_field = 0;
        ...
        test_syntaxes(move |syntax: Syntax| -> Result<(), &str> {
            ...
            test_which_names_can_be_qualified_enum(syntax)?;
