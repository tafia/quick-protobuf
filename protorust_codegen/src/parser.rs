use std::str;
use nom::{alphanumeric, multispace, digit, IError};
...
impl<'a> Field<'a> {
    ...
    fn write_match_tag<W: Write>(&self, w: &mut W) -> IoResult<()> {
        if self.optional {
            writeln!(w, "({}, {}) => msg.{} = Some(r.read_{}()?),",
                self.number, self.wire_type(), self.name, self.read_fn())
        } else if self.repeated {
...
impl<'a> Message<'a> {
    ...
    fn write_impl_message<W: Write>(&self, w: &mut W) -> IoResult<()> {
...
impl<'a> Enumerator<'a> {
    ...
    fn write_impl_message<W: Write>(&self, w: &mut W) -> IoResult<()> {
        ...
        for &(f, number) in &self.fields {
            writeln!(w, "                 ({}, WireType::Varint) => return Ok({}::{}),", 
                     number, self.name, f)?;
        }
        for &(_, number) in &self.fields {
            writeln!(w, "                 ({}, _) => return Err(ErrorKind::InvalidMessage(tag, \"{} enum\").into()),",
                     number, self.name)?;
        }
        writeln!(w, "                (_, wire_type) => r.read_unknown(wire_type)?,")?;
        writeln!(w, "            }}")?;
...
named!(file_descriptor<FileDescriptor>, do_parse!(
...
#[test]
fn test_ignore() {
    let msg = r#"package com.sgcib.i2r.proto.ra.response.V3_0;
