...
impl MessageWrite for Test1 {
    fn get_size(&self) -> usize {
        0 + 1 + sizeof_varint(*(&self.a) as u64)
    }
...
impl MessageWrite for Test3 {
    fn get_size(&self) -> usize {
        0 + 1 + sizeof_len((&self.c).get_size())
    }
...
impl MessageWrite for Test4 {
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        w.write_packed_with_tag(34, &self.d, |w, m| w.write_int32(*m), &|m| {
...
impl MessageWrite for TestPackedUnpacked {
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        for s in &self.unpacked {
            w.write_with_tag(32, |w| w.write_int32(*s))?;
        }
        w.write_packed_with_tag(42, &self.packed, |w, m| w.write_int32(*m), &|m| {
            sizeof_varint(*(m) as u64)
        })?;
        Ok(())
    }
...
impl<'a> MessageRead<'a> for TestSelfReference {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => {
                    msg.r1 = Some(Box::new(r.read_message::<basic::TestSelfReference>(bytes)?))
                }
                Ok(18) => {
                    msg.r2 = Some(Box::new(r.read_message::<basic::TestSelfReference>(bytes)?))
                }
                Ok(t) => {
                    r.read_unknown(bytes, t)?;
                }
...
impl MessageWrite for TestSelfReference {
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if let Some(ref s) = self.r1 {
            w.write_with_tag(10, |w| w.write_message(&**s))?;
        }
        if let Some(ref s) = self.r2 {
...
impl<'a> MessageWrite for TestDefaultInstanceField<'a> {
    fn get_size(&self) -> usize {
        0 + self.s.as_ref().map_or(0, |m| 1 + sizeof_len((m).len()))
    }
...
impl<'a> MessageRead<'a> for TestDefaultInstance<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => {
                    msg.field = Some(r.read_message::<basic::TestDefaultInstanceField>(bytes)?)
                }
                Ok(t) => {
                    r.read_unknown(bytes, t)?;
                }
...
impl<'a> MessageWrite for TestTypesSingular<'a> {
    fn get_size(&self) -> usize {
        0 + self.double_field.as_ref().map_or(0, |_| 1 + 8)
...
impl<'a> MessageWrite for TestTypesRepeated<'a> {
    fn get_size(&self) -> usize {
        0 + (1 + 8) * self.double_field.len()
            ...
            + self
                .sint32_field
                .iter()
                .map(|s| 1 + sizeof_sint32(*(s)))
                .sum::<usize>()
            + self
                .sint64_field
                .iter()
                .map(|s| 1 + sizeof_sint64(*(s)))
                .sum::<usize>()
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        for s in &self.double_field {
            w.write_with_tag(9, |w| w.write_double(*s))?;
        }
        for s in &self.float_field {
            w.write_with_tag(21, |w| w.write_float(*s))?;
        }
        for s in &self.int32_field {
            w.write_with_tag(24, |w| w.write_int32(*s))?;
        }
        for s in &self.int64_field {
            w.write_with_tag(32, |w| w.write_int64(*s))?;
        }
        for s in &self.uint32_field {
            w.write_with_tag(40, |w| w.write_uint32(*s))?;
        }
        for s in &self.uint64_field {
            w.write_with_tag(48, |w| w.write_uint64(*s))?;
        }
        for s in &self.sint32_field {
            w.write_with_tag(56, |w| w.write_sint32(*s))?;
        }
        for s in &self.sint64_field {
            w.write_with_tag(64, |w| w.write_sint64(*s))?;
        }
        for s in &self.fixed32_field {
            w.write_with_tag(77, |w| w.write_fixed32(*s))?;
        }
        for s in &self.fixed64_field {
            w.write_with_tag(81, |w| w.write_fixed64(*s))?;
        }
        for s in &self.sfixed32_field {
            w.write_with_tag(93, |w| w.write_sfixed32(*s))?;
        }
        for s in &self.sfixed64_field {
            w.write_with_tag(97, |w| w.write_sfixed64(*s))?;
        }
        for s in &self.bool_field {
            w.write_with_tag(104, |w| w.write_bool(*s))?;
        }
        for s in &self.string_field {
            w.write_with_tag(114, |w| w.write_string(&**s))?;
        }
        for s in &self.bytes_field {
            w.write_with_tag(122, |w| w.write_bytes(&**s))?;
        }
        for s in &self.enum_field {
            w.write_with_tag(128, |w| w.write_enum(*s as i32))?;
        }
        Ok(())
    }
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(42) => {
                    msg.uint32_field = r.read_packed(bytes, |r, bytes| r.read_uint32(bytes))?
                }
                Ok(50) => {
                    msg.uint64_field = r.read_packed(bytes, |r, bytes| r.read_uint64(bytes))?
                }
                Ok(58) => {
                    msg.sint32_field = r.read_packed(bytes, |r, bytes| r.read_sint32(bytes))?
                }
                Ok(66) => {
                    msg.sint64_field = r.read_packed(bytes, |r, bytes| r.read_sint64(bytes))?
                }
...
impl<'a> MessageWrite for TestTypesRepeatedPacked<'a> {
    fn get_size(&self) -> usize {
        0 + if self.double_field.is_empty() {
            0
        } else {
            1 + sizeof_len(self.double_field.len() * 8)
        } + if self.float_field.is_empty() {
            0
        } else {
            1 + sizeof_len(self.float_field.len() * 4)
            ...
            1 + sizeof_len(
                self.sint64_field
                    .iter()
                    .map(|s| sizeof_sint64(*(s)))
                    .sum::<usize>(),
            )
        } + if self.fixed32_field.is_empty() {
            0
        } else {
            1 + sizeof_len(self.fixed32_field.len() * 4)
        } + if self.fixed64_field.is_empty() {
            0
        } else {
            1 + sizeof_len(self.fixed64_field.len() * 8)
        } + if self.sfixed32_field.is_empty() {
            0
        } else {
            1 + sizeof_len(self.sfixed32_field.len() * 4)
        } + if self.sfixed64_field.is_empty() {
            0
        } else {
            1 + sizeof_len(self.sfixed64_field.len() * 8)
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        w.write_packed_with_tag(26, &self.int32_field, |w, m| w.write_int32(*m), &|m| {
            sizeof_varint(*(m) as u64)
        })?;
        w.write_packed_with_tag(34, &self.int64_field, |w, m| w.write_int64(*m), &|m| {
            sizeof_varint(*(m) as u64)
        })?;
        w.write_packed_with_tag(42, &self.uint32_field, |w, m| w.write_uint32(*m), &|m| {
            sizeof_varint(*(m) as u64)
        })?;
        w.write_packed_with_tag(50, &self.uint64_field, |w, m| w.write_uint64(*m), &|m| {
            sizeof_varint(*(m) as u64)
        })?;
        w.write_packed_with_tag(58, &self.sint32_field, |w, m| w.write_sint32(*m), &|m| {
            sizeof_sint32(*(m))
        })?;
        w.write_packed_with_tag(66, &self.sint64_field, |w, m| w.write_sint64(*m), &|m| {
            sizeof_sint64(*(m))
        })?;
        ...
        w.write_packed_with_tag(106, &self.bool_field, |w, m| w.write_bool(*m), &|m| {
            sizeof_varint(*(m) as u64)
        })?;
        for s in &self.string_field {
            w.write_with_tag(114, |w| w.write_string(&**s))?;
...
impl<'a> MessageWrite for TestTruncated<'a> {
    fn get_size(&self) -> usize {
        0 + if self.ints.is_empty() {
            0
        } else {
            1 + sizeof_len(self.ints.len() * 4)
        }
    }
...
impl MessageWrite for TestBugSint {
    fn get_size(&self) -> usize {
        0 + self.s32.as_ref().map_or(0, |m| 1 + sizeof_sint32(*(m)))
            + self.s64.as_ref().map_or(0, |m| 1 + sizeof_sint64(*(m)))
    }

    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if let Some(ref s) = self.s32 {
            w.write_with_tag(8, |w| w.write_sint32(*s))?;
        }
        if let Some(ref s) = self.s64 {
            w.write_with_tag(16, |w| w.write_sint64(*s))?;
        }
        Ok(())
    }
}
