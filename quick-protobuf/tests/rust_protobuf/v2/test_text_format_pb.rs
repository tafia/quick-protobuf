...
pub struct TestTypes<'a> {
    ...
    pub string_singular: Option<&'a str>,
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(114) => msg.string_singular = Some(r.read_string(bytes)?),
                ...
                Ok(354) => msg.string_repeated.push(r.read_string(bytes)?),
