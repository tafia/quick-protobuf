    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => {
                    msg.imported_message = Some(
                        r.read_message::<test_import_root_imported_pb::ImportedMessage>(bytes)?,
                    )
                }
...
impl MessageWrite for ContainsImported {
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if let Some(ref s) = self.imported_message {
            w.write_with_tag(10, |w| w.write_message(s))?;
        }
        if let Some(ref s) = self.imported_enum {
