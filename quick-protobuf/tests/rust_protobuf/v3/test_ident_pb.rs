    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(10) => {
                    msg.type_pb =
                        mod_TestType::OneOftype_pb::s(r.read_string(bytes).map(Cow::Borrowed)?)
                }
                Ok(t) => {
...
impl<'a> MessageWrite for TestType<'a> {
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        for s in &self.struct_pb {
            w.write_with_tag(18, |w| w.write_string(&**s))?;
        }
        w.write_packed_with_tag(26, &self.ref_pb, |w, m| w.write_uint32(*m), &|m| {
            sizeof_varint(*(m) as u64)
        })?;
        match self.type_pb {
            mod_TestType::OneOftype_pb::s(ref m) => {
                w.write_with_tag(10, |w| w.write_string(&**m))?
            }
