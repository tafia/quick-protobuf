...
impl MessageWrite for MessageForOneof {
    fn get_size(&self) -> usize {
        0 + if self.f == 0i32 {
            ...
            1 + sizeof_varint(*(&self.f) as u64)
        }
    }

    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.f != 0i32 {
            w.write_with_tag(8, |w| w.write_int32(*&self.f))?;
        }
        Ok(())
    }
}
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(93) => {
                    msg.one = mod_TestOneof::OneOfone::sfixed32_field(r.read_sfixed32(bytes)?)
                }
                Ok(97) => {
                    msg.one = mod_TestOneof::OneOfone::sfixed64_field(r.read_sfixed64(bytes)?)
                }
                Ok(104) => msg.one = mod_TestOneof::OneOfone::bool_field(r.read_bool(bytes)?),
                Ok(114) => {
                    msg.one = mod_TestOneof::OneOfone::string_field(
                        r.read_string(bytes).map(Cow::Borrowed)?,
                    )
                }
                Ok(122) => {
                    msg.one = mod_TestOneof::OneOfone::bytes_field(
                        r.read_bytes(bytes).map(Cow::Borrowed)?,
                    )
                }
                Ok(128) => msg.one = mod_TestOneof::OneOfone::enum_field(r.read_enum(bytes)?),
                Ok(138) => {
                    msg.one = mod_TestOneof::OneOfone::message_field(
                        r.read_message::<MessageForOneof>(bytes)?,
                    )
                }
                Ok(t) => {
                    r.read_unknown(bytes, t)?;
...
impl<'a> MessageWrite for TestOneof<'a> {
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.s != Cow::Borrowed("") {
            w.write_with_tag(234, |w| w.write_string(&**&self.s))?;
        }
        match self.one {
            mod_TestOneof::OneOfone::double_field(ref m) => {
                w.write_with_tag(9, |w| w.write_double(*m))?
            }
            mod_TestOneof::OneOfone::float_field(ref m) => {
                w.write_with_tag(21, |w| w.write_float(*m))?
            }
            mod_TestOneof::OneOfone::int32_field(ref m) => {
                w.write_with_tag(24, |w| w.write_int32(*m))?
            }
            mod_TestOneof::OneOfone::int64_field(ref m) => {
                w.write_with_tag(32, |w| w.write_int64(*m))?
            }
            mod_TestOneof::OneOfone::uint32_field(ref m) => {
                w.write_with_tag(40, |w| w.write_uint32(*m))?
            }
            mod_TestOneof::OneOfone::uint64_field(ref m) => {
                w.write_with_tag(48, |w| w.write_uint64(*m))?
            }
            mod_TestOneof::OneOfone::sint32_field(ref m) => {
                w.write_with_tag(56, |w| w.write_sint32(*m))?
            }
            mod_TestOneof::OneOfone::sint64_field(ref m) => {
                w.write_with_tag(64, |w| w.write_sint64(*m))?
            }
            mod_TestOneof::OneOfone::fixed32_field(ref m) => {
                w.write_with_tag(77, |w| w.write_fixed32(*m))?
            }
            mod_TestOneof::OneOfone::fixed64_field(ref m) => {
                w.write_with_tag(81, |w| w.write_fixed64(*m))?
            }
            mod_TestOneof::OneOfone::sfixed32_field(ref m) => {
                w.write_with_tag(93, |w| w.write_sfixed32(*m))?
            }
            mod_TestOneof::OneOfone::sfixed64_field(ref m) => {
                w.write_with_tag(97, |w| w.write_sfixed64(*m))?
            }
            mod_TestOneof::OneOfone::bool_field(ref m) => {
                w.write_with_tag(104, |w| w.write_bool(*m))?
            }
            mod_TestOneof::OneOfone::string_field(ref m) => {
                w.write_with_tag(114, |w| w.write_string(&**m))?
            }
            mod_TestOneof::OneOfone::bytes_field(ref m) => {
                w.write_with_tag(122, |w| w.write_bytes(&**m))?
            }
            mod_TestOneof::OneOfone::enum_field(ref m) => {
                w.write_with_tag(128, |w| w.write_enum(*m as i32))?
            }
            mod_TestOneof::OneOfone::message_field(ref m) => {
                w.write_with_tag(138, |w| w.write_message(m))?
            }
