...
impl MessageWrite for TestEmpty {
    fn get_size(&self) -> usize {
        0 + if self.foo == 0i32 {
            ...
            1 + sizeof_varint(*(&self.foo) as u64)
        }
    }

    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.foo != 0i32 {
            w.write_with_tag(80, |w| w.write_int32(*&self.foo))?;
        }
        Ok(())
    }
}
...
impl<'a> MessageWrite for TestDefaultInstanceField<'a> {
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.s != Cow::Borrowed("") {
            w.write_with_tag(10, |w| w.write_string(&**&self.s))?;
        }
        Ok(())
    }
}
...
impl MessageWrite for TestDescriptor {
    fn get_size(&self) -> usize {
        0 + if self.stuff == 0i32 {
            0
        } else {
            1 + sizeof_varint(*(&self.stuff) as u64)
        }
    }

    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.stuff != 0i32 {
            w.write_with_tag(80, |w| w.write_int32(*&self.stuff))?;
        }
        Ok(())
    }
}
...
impl<'a> MessageWrite for TestTypesSingular<'a> {
    fn get_size(&self) -> usize {
        0 + if self.double_field == 0f64 { 0 } else { 1 + 8 }
            ...
            + if self.sfixed32_field == 0i32 {
                0
            } else {
                1 + 4
            }
            + if self.sfixed64_field == 0i64 {
                0
            } else {
                1 + 8
            }
            ...
            + if self.string_field == Cow::Borrowed("") {
                0
            } else {
                1 + sizeof_len((&self.string_field).len())
            }
            + if self.bytes_field == Cow::Borrowed(b"") {
                0
            } else {
                1 + sizeof_len((&self.bytes_field).len())
            }
            + if self.enum_field == basic::TestEnumDescriptor::UNKNOWN {
                0
            } else {
...
impl MessageWrite for TestBugSint {
    fn get_size(&self) -> usize {
        0 + if self.s32 == 0i32 {
            ...
            1 + sizeof_sint32(*(&self.s32))
        } + if self.s64 == 0i64 {
            0
        } else {
            1 + sizeof_sint64(*(&self.s64))
        }
    }

    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.s32 != 0i32 {
            w.write_with_tag(8, |w| w.write_sint32(*&self.s32))?;
        }
        if self.s64 != 0i64 {
            w.write_with_tag(16, |w| w.write_sint64(*&self.s64))?;
        }
        Ok(())
    }
}
