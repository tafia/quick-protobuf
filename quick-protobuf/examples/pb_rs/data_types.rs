...
impl<'a> FooMessage<'a> {
    pub const DEFAULT_F_SINT64: i64 = 4i64;
    pub const DEFAULT_F_BOOL: bool = true;
    pub const DEFAULT_F_FIXED32: u32 = 0u32;
}
...
impl<'a> Default for FooMessage<'a> {
    fn default() -> Self {
        Self {
            ...
            f_repeated_packed_float: Cow::from(Vec::new()),
            f_imported: None,
            f_baz: None,
            f_nested: None,
            f_nested_enum: None,
...
impl<'a> MessageWrite for FooMessage<'a> {
    fn get_size(&self) -> usize {
        0
        + if self.f_int32.is_some() { 1 + sizeof_varint((*(self.f_int32.as_ref().unwrap())) as u64) } else { 0 }
        + if self.f_int64.is_some() { 1 + sizeof_varint((*(self.f_int64.as_ref().unwrap())) as u64) } else { 0 }
        + if self.f_uint32.is_some() { 1 + sizeof_varint((*(self.f_uint32.as_ref().unwrap())) as u64) } else { 0 }
        + if self.f_uint64.is_some() { 1 + sizeof_varint((*(self.f_uint64.as_ref().unwrap())) as u64) } else { 0 }
        + if self.f_sint32.is_some() { 1 + sizeof_sint32((*(self.f_sint32.as_ref().unwrap()))) } else { 0 }
        + if self.f_sint64.is_some() { 1 + sizeof_sint64((*(self.f_sint64.as_ref().unwrap()))) } else { 0 }
        + if self.f_bool.is_some() { 1 + sizeof_varint((*(self.f_bool.as_ref().unwrap())) as u64) } else { 0 }
        + if self.f_FooEnum.is_some() { 1 + sizeof_varint((*(self.f_FooEnum.as_ref().unwrap())) as u64) } else { 0 }
        + if self.f_fixed64.is_some() { 1 + 8 } else { 0 }
        + if self.f_sfixed64.is_some() { 1 + 8 } else { 0 }
        + if self.f_fixed32.is_some() { 1 + 4 } else { 0 }
        + if self.f_sfixed32.is_some() { 1 + 4 } else { 0 }
        + if self.f_double.is_some() { 1 + 8 } else { 0 }
        + if self.f_float.is_some() { 1 + 4 } else { 0 }
        + if self.f_bytes.is_some() { 1 + sizeof_len((self.f_bytes.as_ref().unwrap()).len()) } else { 0 }
        + if self.f_string.is_some() { 2 + sizeof_len((self.f_string.as_ref().unwrap()).len()) } else { 0 }
        + if self.f_self_message.is_some() { 2 + sizeof_len((*(self.f_self_message.as_ref().unwrap())).get_size()) } else { 0 }
        + if self.f_bar_message.is_some() { 2 + sizeof_len((self.f_bar_message.as_ref().unwrap()).get_size()) } else { 0 }
        ...
        + if self.f_imported.is_some() { 2 + sizeof_len((self.f_imported.as_ref().unwrap()).get_size()) } else { 0 }
        + if self.f_baz.is_some() { 2 + sizeof_len((self.f_baz.as_ref().unwrap()).get_size()) } else { 0 }
        + if self.f_nested.is_some() { 2 + sizeof_len((self.f_nested.as_ref().unwrap()).get_size()) } else { 0 }
        + if self.f_nested_enum.is_some() { 2 + sizeof_varint((*(self.f_nested_enum.as_ref().unwrap())) as u64) } else { 0 }
        + self.f_map.iter().map(|(k, &v)| 2 + sizeof_len(2 + sizeof_len((k).len()) + sizeof_varint((v) as u64))).sum::<usize>()
        + match &self.test_oneof {
            mod_FooMessage::OneOftest_oneof::f1(m) => 2 + sizeof_varint((*m) as u64),
            mod_FooMessage::OneOftest_oneof::f2(m) => 2 + sizeof_varint((*m) as u64),
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.f_int32.is_some() { w.write_with_tag(8, |w| w.write_int32(*(self.f_int32.as_ref().unwrap())))?; }
        if self.f_int64.is_some() { w.write_with_tag(16, |w| w.write_int64(*(self.f_int64.as_ref().unwrap())))?; }
        if self.f_uint32.is_some() { w.write_with_tag(24, |w| w.write_uint32(*(self.f_uint32.as_ref().unwrap())))?; }
        if self.f_uint64.is_some() { w.write_with_tag(32, |w| w.write_uint64(*(self.f_uint64.as_ref().unwrap())))?; }
        if self.f_sint32.is_some() { w.write_with_tag(40, |w| w.write_sint32(*(self.f_sint32.as_ref().unwrap())))?; }
        if self.f_sint64.is_some() { w.write_with_tag(48, |w| w.write_sint64(*(self.f_sint64.as_ref().unwrap())))?; }
        if self.f_bool.is_some() { w.write_with_tag(56, |w| w.write_bool(*(self.f_bool.as_ref().unwrap())))?; }
        if self.f_FooEnum.is_some() { w.write_with_tag(64, |w| w.write_enum(*(self.f_FooEnum.as_ref().unwrap()) as i32))?; }
        if self.f_fixed64.is_some() { w.write_with_tag(73, |w| w.write_fixed64(*(self.f_fixed64.as_ref().unwrap())))?; }
        if self.f_sfixed64.is_some() { w.write_with_tag(81, |w| w.write_sfixed64(*(self.f_sfixed64.as_ref().unwrap())))?; }
        if self.f_fixed32.is_some() { w.write_with_tag(93, |w| w.write_fixed32(*(self.f_fixed32.as_ref().unwrap())))?; }
        if self.f_sfixed32.is_some() { w.write_with_tag(101, |w| w.write_sfixed32(*(self.f_sfixed32.as_ref().unwrap())))?; }
        if self.f_double.is_some() { w.write_with_tag(105, |w| w.write_double(*(self.f_double.as_ref().unwrap())))?; }
        if self.f_float.is_some() { w.write_with_tag(117, |w| w.write_float(*(self.f_float.as_ref().unwrap())))?; }
        if self.f_bytes.is_some() { w.write_with_tag(122, |w| w.write_bytes(&self.f_bytes.as_ref().unwrap()))?; }
        if self.f_string.is_some() { w.write_with_tag(130, |w| w.write_string(&self.f_string.as_ref().unwrap()))?; }
        if self.f_self_message.is_some() { w.write_with_tag(138, |w| w.write_message(&*(*(self.f_self_message.as_ref().unwrap()))))?; }
        if self.f_bar_message.is_some() { w.write_with_tag(146, |w| w.write_message(self.f_bar_message.as_ref().unwrap()))?; }
        ...
        if self.f_imported.is_some() { w.write_with_tag(178, |w| w.write_message(self.f_imported.as_ref().unwrap()))?; }
        if self.f_baz.is_some() { w.write_with_tag(186, |w| w.write_message(self.f_baz.as_ref().unwrap()))?; }
        if self.f_nested.is_some() { w.write_with_tag(194, |w| w.write_message(self.f_nested.as_ref().unwrap()))?; }
        if self.f_nested_enum.is_some() { w.write_with_tag(200, |w| w.write_enum(*(self.f_nested_enum.as_ref().unwrap()) as i32))?; }
        ...
        match &self.test_oneof {
            mod_FooMessage::OneOftest_oneof::f1(m) => { w.write_with_tag(216, |w| w.write_int32(*m))? },
            mod_FooMessage::OneOftest_oneof::f2(m) => { w.write_with_tag(224, |w| w.write_bool(*m))? },
            mod_FooMessage::OneOftest_oneof::f3(m) => { w.write_with_tag(234, |w| w.write_string(m))? },
...
impl MessageWrite for BazMessage {
    fn get_size(&self) -> usize {
        0
        + if self.nested.is_some() { 1 + sizeof_len((self.nested.as_ref().unwrap()).get_size()) } else { 0 }
    }

    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.nested.is_some() { w.write_with_tag(10, |w| w.write_message(self.nested.as_ref().unwrap()))?; }
        Ok(())
    }
}
...
impl Default for Nested {
    fn default() -> Self {
        Self {
            f_nested: mod_BazMessage::mod_Nested::NestedMessage::default(),
        }
    }
}
