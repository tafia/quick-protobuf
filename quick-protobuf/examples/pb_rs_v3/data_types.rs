    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(224) => {
                    msg.test_oneof = mod_FooMessage::OneOftest_oneof::f2(r.read_bool(bytes)?)
                }
                Ok(234) => {
                    msg.test_oneof = mod_FooMessage::OneOftest_oneof::f3(
...
impl<'a> MessageWrite for FooMessage<'a> {
    fn get_size(&self) -> usize {
        0 + if self.f_int32 == 0i32 {
            ...
            + if self.f_repeated_packed_float.is_empty() {
                ...
                2 + sizeof_len(self.f_repeated_packed_float.len() * 4)
            }
            + self
                .f_imported
                .as_ref()
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if let Some(ref s) = self.f_imported {
            w.write_with_tag(178, |w| w.write_message(s))?;
        }
        if let Some(ref s) = self.f_baz {
            w.write_with_tag(186, |w| w.write_message(s))?;
        }
        ...
        match self.test_oneof {
            mod_FooMessage::OneOftest_oneof::f1(ref m) => {
                w.write_with_tag(216, |w| w.write_int32(*m))?
            }
            mod_FooMessage::OneOftest_oneof::f2(ref m) => {
                w.write_with_tag(224, |w| w.write_bool(*m))?
            }
            mod_FooMessage::OneOftest_oneof::f3(ref m) => {
                w.write_with_tag(234, |w| w.write_string(&**m))?
            }
