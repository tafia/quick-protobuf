...
impl MessageWrite for BarMessage {
    fn get_size(&self) -> usize {
        0
        + if self.b_int32 != 0i32 { 1 + sizeof_varint((self.b_int32) as u64) } else { 0 }
    }
...
pub struct FooMessage<'a> {
    ...
    pub f_imported: a::b::ImportedMessage,
    pub f_baz: BazMessage<'a>,
    pub f_nested: mod_BazMessage::Nested,
...
impl<'a> Default for FooMessage<'a> {
    fn default() -> Self {
        Self {
            ...
            f_sint64: 0i64,
            f_bool: false,
            f_FooEnum: data_types::FooEnum::FIRST_VALUE,
            f_fixed64: 0u64,
            f_sfixed64: 0i64,
            f_fixed32: 0u32,
            f_sfixed32: 0i32,
            f_double: 0f64,
            f_float: 0f32,
            f_bytes: Cow::Borrowed(b""),
            f_string: Cow::Borrowed(""),
            f_self_message: None,
            ...
            f_imported: a::b::ImportedMessage::default(),
            f_baz: BazMessage::default(),
            f_nested: mod_BazMessage::Nested::default(),
            f_nested_enum: data_types::mod_BazMessage::mod_Nested::NestedEnum::Foo,
            f_map: HashMap::new(),
            f_repeated_string: Vec::new(),
            f_repeated_baz_message: Vec::new(),
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(146) => msg.f_bar_message = r.read_message::<BarMessage>(bytes)?,
                ...
                Ok(178) => msg.f_imported = r.read_message::<a::b::ImportedMessage>(bytes)?,
                Ok(186) => msg.f_baz = r.read_message::<BazMessage>(bytes)?,
                Ok(194) => msg.f_nested = r.read_message::<mod_BazMessage::Nested>(bytes)?,
...
impl<'a> MessageWrite for FooMessage<'a> {
    fn get_size(&self) -> usize {
        0
        + if self.f_int32 != 0i32 { 1 + sizeof_varint((self.f_int32) as u64) } else { 0 }
        + if self.f_int64 != 0i64 { 1 + sizeof_varint((self.f_int64) as u64) } else { 0 }
        + if self.f_uint32 != 0u32 { 1 + sizeof_varint((self.f_uint32) as u64) } else { 0 }
        + if self.f_uint64 != 0u64 { 1 + sizeof_varint((self.f_uint64) as u64) } else { 0 }
        + if self.f_sint32 != 0i32 { 1 + sizeof_sint32((self.f_sint32)) } else { 0 }
        + if self.f_sint64 != 0i64 { 1 + sizeof_sint64((self.f_sint64)) } else { 0 }
        + if self.f_bool != false { 1 + sizeof_varint((self.f_bool) as u64) } else { 0 }
        + if self.f_FooEnum != data_types::FooEnum::FIRST_VALUE { 1 + sizeof_varint((self.f_FooEnum) as u64) } else { 0 }
        + if self.f_fixed64 != 0u64 { 1 + 8 } else { 0 }
        + if self.f_sfixed64 != 0i64 { 1 + 8 } else { 0 }
        + if self.f_fixed32 != 0u32 { 1 + 4 } else { 0 }
        + if self.f_sfixed32 != 0i32 { 1 + 4 } else { 0 }
        + if self.f_double != 0f64 { 1 + 8 } else { 0 }
        + if self.f_float != 0f32 { 1 + 4 } else { 0 }
        + if self.f_bytes != Cow::Borrowed(b"") { 1 + sizeof_len((self.f_bytes).len()) } else { 0 }
        + if self.f_string != Cow::Borrowed("") { 2 + sizeof_len((self.f_string).len()) } else { 0 }
        + if self.f_self_message.is_some() { 2 + sizeof_len((*(self.f_self_message.as_ref().unwrap())).get_size()) } else { 0 }
        + 2 + sizeof_len((self.f_bar_message).get_size())
        ...
        + 2 + sizeof_len((self.f_imported).get_size())
        + 2 + sizeof_len((self.f_baz).get_size())
        + 2 + sizeof_len((self.f_nested).get_size())
        + if self.f_nested_enum != data_types::mod_BazMessage::mod_Nested::NestedEnum::Foo { 2 + sizeof_varint((self.f_nested_enum) as u64) } else { 0 }
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        w.write_with_tag(146, |w| w.write_message(&self.f_bar_message))?;
        ...
        w.write_with_tag(178, |w| w.write_message(&self.f_imported))?;
        w.write_with_tag(186, |w| w.write_message(&self.f_baz))?;
        w.write_with_tag(194, |w| w.write_message(&self.f_nested))?;
...
pub struct BazMessage<'a> {
    pub nested: mod_BazMessage::Nested,
    pub b_int64: i64,
    pub b_string: Cow<'a, str>,
}


impl<'a> Default for BazMessage<'a> {
    fn default() -> Self {
        Self {
            nested: mod_BazMessage::Nested::default(),
            b_int64: 0i64,
            b_string: Cow::Borrowed(""),
        }
    }
}
...
impl<'a> MessageRead<'a> for BazMessage<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.nested = r.read_message::<mod_BazMessage::Nested>(bytes)?,
...
impl<'a> MessageWrite for BazMessage<'a> {
    fn get_size(&self) -> usize {
        0
        + 1 + sizeof_len((self.nested).get_size())
        + if self.b_int64 != 0i64 { 1 + sizeof_varint((self.b_int64) as u64) } else { 0 }
        + if self.b_string != Cow::Borrowed("") { 1 + sizeof_len((self.b_string).len()) } else { 0 }
    }

    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        w.write_with_tag(10, |w| w.write_message(&self.nested))?;
...
impl MessageWrite for NestedMessage {
    fn get_size(&self) -> usize {
        0
        + if self.f_nested != 0i32 { 1 + sizeof_varint((self.f_nested) as u64) } else { 0 }
    }
